  ...
  try {
    ...
    if (allSkills.length === 0) {
      return {
        name: CONFIG_PATH,
        status: "warn",
        detail: "no skills are required anywhere - hook will never inject reminders",
        fix: `Add at least one skill to requiredSkills or rules[].requiredSkills in ${CONFIG_PATH}`,
        why: WHY_CONFIG,
      };
    }
    return {
      name: CONFIG_PATH,
      status: "pass",
      detail: `skills that can be required: ${JSON.stringify(allSkills)}`,
      why: WHY_CONFIG,
    };
  } catch (e) {
    return {
...
const BYPASS_WORKFLOW_PATH = ".github/workflows/skilled-pr-bypass.yml";
const WHY_BYPASS_WORKFLOW =
  "Rules that bypass or replace requiredSkills need a GitHub Actions workflow to post the matching 'not required' success statuses. Branch protection can require the union of all possible contexts, but without this workflow some PRs wait forever on a context no review skill will post.";

function rulesCanChangeRequiredSkills(config: SkilledPRConfig): boolean {
  return config.rules.some((rule) => rule.requiredSkills !== undefined);
}

export function classifyBypassWorkflow(
  workflowContent: string | null,
  config: SkilledPRConfig | null,
): CheckResult {
  if (config === null) {
    return {
  ...
  if (!rulesCanChangeRequiredSkills(config)) {
    return {
      name: "Bypass workflow",
      status: "skip",
      detail: "no rules override requiredSkills",
      why: WHY_BYPASS_WORKFLOW,
    };
  }
  if (workflowContent === null) {
    return {
      name: "Bypass workflow",
      status: "warn",
      detail: `${BYPASS_WORKFLOW_PATH} not found`,
      fix: `skilled-pr enable-gate, then commit and push ${BYPASS_WORKFLOW_PATH} to the default branch`,
      why: WHY_BYPASS_WORKFLOW,
    };
  }
  ...
  const missing = requiredSnippets.filter((snippet) => !workflowContent.includes(snippet));
  if (missing.length > 0) {
    return {
      name: "Bypass workflow",
      status: "warn",
      detail: `${BYPASS_WORKFLOW_PATH} is missing expected v1 rule-status behavior`,
      fix: `skilled-pr enable-gate, then commit and push ${BYPASS_WORKFLOW_PATH} to the default branch`,
      why: WHY_BYPASS_WORKFLOW,
    };
  }
  return {
    name: "Bypass workflow",
    status: "pass",
    detail: `${BYPASS_WORKFLOW_PATH} present`,
    why: WHY_BYPASS_WORKFLOW,
  };
}
...
export async function doctor(args: string[] = []) {
  ...
  // 5e. Generated bypass workflow. Only required when rules can change
  //     requiredSkills, because branch protection then needs ci-resolve to
  //     release unused contexts with "not required" statuses.
  results.push(
    classifyBypassWorkflow(
      await readFileOrNull(BYPASS_WORKFLOW_PATH),
      parsedConfig,
    ),
  );

  // 5f. Referenced skills exist somewhere.
  if (parsedConfig !== null) {
